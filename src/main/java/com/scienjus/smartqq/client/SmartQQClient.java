...
public class SmartQQClient {
    ...
    //拉取消息的线程
    public ScheduledFuture pollMessageFuture;
    ...
    public void getQRCode() {
        ...
        try (CloseableHttpResponse response = client.execute(get, context);
            ...
            out.close();
            LOGGER.println("[INFO] 二维码已保存在 " + filePath + " 文件中，请打开手机QQ并扫描二维码");
        } catch (IOException e) {
            LOGGER.println("[ERROR] 获取二维码失败");
        }
    }

    //登录流程2：校验二维码
    public String verifyQRCode() {
        LOGGER.println("[INFO] 等待扫描二维码");
        ...
        while (true) {
            ...
            try (CloseableHttpResponse response = client.execute(get, context)) {
                ...
                if (responseText.contains("成功")) {
                    ...
                    LOGGER.println("[INFO] 二维码已失效，尝试重新获取二维码");
                    getQRCode();
                }
            } catch (IOException e) {
                LOGGER.println("[ERROR] 校验二维码失败");
            }
        }
    ...
    public void getPtwebqq(String url) {
        LOGGER.println("[INFO] 开始获取ptwebqq");
        //发送请求的客户端
        CloseableHttpClient client = HttpClients.createDefault();
        ...
        try {
            ...
            LOGGER.println("[ERROR] 获取ptwebqq失败");
        }
    }

    //登录流程4：获取vfwebqq
    public void getVfwebqq() {
        LOGGER.println("[INFO] 开始获取vfwebqq");
        //发送请求的客户端
        CloseableHttpClient client = HttpClients.createDefault();
        ...
        try (CloseableHttpResponse response = client.execute(get, context)) {
            ...
            LOGGER.println("[ERROR] 获取vfwebqq失败");
        }
    }

    //登录流程5：获取uin和psessionid
    public void getUinAndPsessionid() {
        LOGGER.println("[INFO] 开始获取uin和psessionid");
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            LOGGER.println("[ERROR] 获取uin和psessionid失败");
        }
    }
    ...
    public void sendMessageToGroup(long groupId, String msg) {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            if (result != null && result == 0) {
                LOGGER.println("[INFO] 发送群消息成功");
            } else {
                LOGGER.println("[ERROR] 发送群消息失败 返回码：" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 发送群消息失败");
        }
    }
    ...
    public void sendMessageToDiscuss(long discussId, String msg) {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            Integer result = responseJson.getInteger("errCode");
            if (result != null && result == 0) {
                LOGGER.println("[INFO] 发送讨论组消息成功");
            } else {
                LOGGER.println("[ERROR] 发送讨论组消息失败 返回码：" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 发送群消息失败");
        }
    ...
    public void sendMessageToFriend(long friendId, String msg) {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            Integer result = responseJson.getInteger("errCode");
            if (result != null && result == 0) {
                LOGGER.println("[INFO] 发送消息成功");
            } else {
                LOGGER.println("[ERROR] 发送消息失败 返回码：" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 发送消息失败");
        }
    }
    ...
    public List<Discuss> getDiscussList() {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            if (0 == responseJson.getIntValue("retcode")) {
                ...
                LOGGER.println("[ERROR] 获取讨论组列表失败");
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 获取讨论组列表失败");
        }
    ...
    private static Map<Long, Friend> parseFriendMap(JSONObject result) {
        ...
        for (int i = 0; marknames != null && i < marknames.size(); i++) {
            ...
            friendMap.get(item.getLongValue("uin")).markname = item.getString("markname");
        }
        ...
        for (int i = 0; vipinfo != null && i < vipinfo.size(); i++) {
            ...
            friend.vip = (item.getIntValue("is_vip") == 1);
    ...
    public UserInfo getAccountInfo() {
        LOGGER.println("[INFO] 开始获取登录用户信息");
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            if (0 == responseJson.getIntValue("retcode")) {
                ...
                LOGGER.println("[ERROR] 获取登录用户信息失败 retcode:" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 获取登录用户信息失败");
        }
        return null;
    }
    ...
    public UserInfo getFriendInfo(long friendId) {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            if (0 == responseJson.getIntValue("retcode")) {
                ...
                LOGGER.println("[ERROR] 获取好友信息失败 retcode:" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 获取好友信息失败");
        }
        return null;
    }
    ...
    public List<Recent> getRecentList() {
        LOGGER.println("[INFO] 开始获取最近会话列表");
        JSONObject r = new JSONObject();
        r.put("vfwebqq", vfwebqq);
        r.put("clientid", Client_ID);
        r.put("psessionid", "");
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            if (0 == responseJson.getIntValue("retcode")) {
                ...
                LOGGER.println("[ERROR] 获取最近会话列表失败 retcode:" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 获取最近会话列表失败");
        }
        return null;
    }
    ...
    public long getQQById(long id) {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            if (0 == responseJson.getIntValue("retcode")) {
                ...
                LOGGER.println("[ERROR] 获取QQ号失败 retcode:" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 获取QQ号失败");
        }
        return 0L;
    }
    ...
    public List<FriendStatus> getFriendStatus() {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            if (0 == responseJson.getIntValue("retcode")) {
                ...
                LOGGER.println("[ERROR] 获取好友状态失败 retcode:" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 获取好友状态失败");
        }
        return null;
    }
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            if (0 == responseJson.getIntValue("retcode")) {
                ...
                for (int i = 0; cards != null && i < cards.size(); i++) {
                    ...
                    groupUserMap.get(item.getLongValue("muin")).card = item.getString("card");
                }
                JSONArray vipinfo = result.getJSONArray("vipinfo");
                for (int i = 0; vipinfo != null && i < vipinfo.size(); i++) {
                    JSONObject item = vipinfo.getJSONObject(i);
                    GroupUser groupUser = groupUserMap.get(item.getLongValue("u"));
                    groupUser.vip = (item.getIntValue("is_vip") == 1);
                    groupUser.vipLevel = item.getIntValue("vip_level");
                }
                return groupInfo;
            } else {
                LOGGER.println("[ERROR] 获取群资料失败 retcode:" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 获取群资料失败");
        }
        return null;
    }
    ...
    public DiscussInfo getDiscussInfo(long discussId) {
        ...
        try (CloseableHttpClient client = HttpClients.createDefault();
            ...
            if (0 == responseJson.getIntValue("retcode")) {
                ...
                LOGGER.println("[ERROR] 获取讨论组资料失败 retcode:" + responseJson.getIntValue("retcode"));
            }
        } catch (IOException e) {
            LOGGER.println("[ERROR] 获取讨论组资料失败");
        }
        return null;
    }
        ...
        public void run() {
            try (CloseableHttpClient client = HttpClients.createDefault();
                ...
                if (responseJson.getIntValue("retcode") == 0) {
                    ...
                    LOGGER.println("[ERROR] 接受消息失败 retcode: " + responseJson.getIntValue("retcode"));
                }
            } catch (IOException e) {
                LOGGER.println("[INFO] 暂时没有消息");
            }
        }
    }
}
